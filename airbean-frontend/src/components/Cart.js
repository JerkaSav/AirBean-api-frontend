...
import checkIfDiscount from '../functions/discountFunction';
      ...
      <div className="cart-inner">
        ...
        <ul>
          {order.map((item, index) => {
            return (
              <CartItems key={index} title={item.title} price={item.price} amount={item.amount} update={updateCart} />
            );
          })}
        </ul>
        <footer>
          <div className="total">
            ...
            {total.discount ? (
              <Fragment>
                <h2 className="red">{total.totalDiscount} Kr</h2> <h2 className="line-big">{total.total} Kr</h2>
              </Fragment>
            ) : (
          ...
          <div className="btn-wrapper">
            <Btn text="Take my money!" selector="btn-dark" update={sendOrder} />
          </div>
        </footer>
      </div>
